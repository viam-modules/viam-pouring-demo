...
func (g *gen) demoPlanMovements(bottleGrabPoint r3.Vector, cupLocations []r3.Vector) {
	...
	bottlePlan, err := getPlan(context.Background(), logger, g.r, armFrameApproachGoalInputs[len(armFrameApproachGoalInputs)-1], gripperResource, bottlegoal, worldState, &linearAndBottleConstraint, 0)
	if err != nil {
		logger.Fatal(err)
	}
	...
	liftedPlan, err := getPlan(context.Background(), logger, g.r, armFrameBottlePlanInputs[len(armFrameBottlePlanInputs)-1], gripperResource, liftedgoal, worldState, &bottleGripperSpec, 0)
	if err != nil {
		logger.Fatal(err)
	}
	...
	for i, cupLoc := range cupLocations {
		...
		if i == 0 {
			...
			plan, err = getPlan(context.Background(), logger, g.r, armFrameLiftedPlanInputs[len(armFrameLiftedPlanInputs)-1], bottleResource, pourReadyGoal, worldState, orientationConstraint, 0)
			if err != nil {
				logger.Fatal(err)
			}
			if getBackHomeCachedPlan == nil {
				...
				for {
					...
					g.logger.Info(" ")
					g.logger.Info(" ")
					plan, err = getPlan(context.Background(), logger, g.r, armFrameLiftedPlanInputs[len(armFrameLiftedPlanInputs)-1], bottleResource, pourReadyGoal, worldState, orientationConstraint, j)
					if err != nil {
						logger.Fatal(err)
					}
					j++
			...
			plan, err = getPlan(context.Background(), logger, g.r, armFrameFormerPlanInputs[len(armFrameFormerPlanInputs)-1], bottleResource, pourReadyGoal, worldState, orientationConstraint, 0)
			if err != nil {
				logger.Fatal(err)
			}
		}
		...
		plan, err = getPlan(context.Background(), logger, g.r, armFramePlanInputs[len(armFramePlanInputs)-1], bottleResource, pourGoal, worldState, &linearConstraint, 0)
		if err != nil {
			logger.Fatal(err)
		}
...
func getPlan(ctx context.Context, logger logging.Logger, r robot.Robot, armCurrentInputs []referenceframe.Input, toMove resource.Name, goal spatialmath.Pose, worldState *referenceframe.WorldState, constraint *motionplan.Constraints, rseed int) (motionplan.Plan, error) {
	fsCfg, _ := r.FrameSystemConfig(ctx)
	parts := fsCfg.Parts
