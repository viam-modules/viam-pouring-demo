...
// const (
// 	// todo: these should become memebers of the config for ease of adjustment
// 	deltaXNeg = 0.295
// 	deltaXPos = 0.225
// 	deltaYNeg = 0.295
// 	deltaYPos = 0.
...
// 		g.logger.Infof("old x: %f", x)
// 		g.logger.Infof("old y: %f", y)
// 		xAdj, yAdj := g.determineAdjustment(x, y)
...
// 		vec := circleToPt(*properties.IntrinsicParams, x, y, cameraToTable, xAdj, yAdj)
// 		res = append(res, spatialmath.NewPoseFromPoint(vec))
// 	}
...
// 	deltaYPos := deltaYPos
// 	deltaYNeg := deltaYNeg
// 	g.logger.Infof("inputX: %f", inputX)
// 	g.logger.Infof("inputY: %f", inputY)
// 	g.logger.Infof("deltaXPos: %f", deltaXPos)
// 	g.logger.Infof("deltaYPos: %f", deltaYPos)
// 	g.logger.Infof("deltaXNeg: %f", deltaXNeg)
// 	g.logger.Infof("deltaYNeg: %f", deltaYNeg)
...
// func circleToPt(intrinsics transform.PinholeCameraIntrinsics, x, y, z, xAdjustment, yAdjustment float64) r3.Vector {
// 	xmm := (float64(x) - intrinsics.Ppx) * (z / intrinsics.Fx)
// 	ymm := (float64(y) - intrinsics.Ppy) * (z / intrinsics.Fy)
// 	xmm = xmm + xAdjustment
// 	ymm = ymm + yAdjustment
